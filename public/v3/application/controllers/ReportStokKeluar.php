...
class ReportStokKeluar extends CI_Controller {
    ...
    public function index()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if ($this->input->post('filter')) {
                $data['title']        = 'Laporan';
                $data['title2']       = 'Laporan Pengeluaran Stok';
                $data['stokKeluar']        = $this->M_order->report_stok_keluar($dari_tanggal, $sampai_tanggal)->result_array();
                ...
                $data['stokKeluar']        = $this->M_order->report_stok_keluar($dari_tanggal, $sampai_tanggal)->result_array();
                $this->load->view('report/cetakStokKeluar', $data);
            }
        }
    }
