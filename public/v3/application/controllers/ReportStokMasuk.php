...
class ReportStokMasuk extends CI_Controller {
	...
	// {
	// 	$data['title']				= 'Laporan';
	// 	$data['title2']				= 'Laporan Pemasukan Stok';
	// 	$this->load->view('report/stokMasuk', $data);
	// }
    ...
    public function server_side()
    {
        ...
        $totalData 		= $this->M_barang->get_all_barang_masuk()->num_rows();
        $totalFiltered  = $totalData;

        if (!empty($this->input->post('search')['value'])) {
            $search = $this->input->post('search')['value'];
            $gs =  $this->M_barang->get_all_barang_masuk($limit, $start, $search)->result_array();
            $totalFiltered = $this->M_barang->count_all_barang_masuk($search);
        } else {
            $gs = $this->M_barang->get_all_barang_masuk($limit, $start)->result_array();
        }
        ...
        if (!empty($gs)) {
            foreach ($gs as $key => $row) {
                ...
                $nestedData['tanggal_masuk'] = date('d F Y', strtotime($row['tanggal_masuk']));
    ...
    public function index()
    {
        ...
        if (!$this->form_validation->run()) {
            ...
            if ($this->input->post('filter')) {
                $data['title']        = 'Laporan';
                $data['title2']       = 'Laporan Pemasukan Stok';
                $data['stokMasuk']        = $this->M_barang->report_barang_masuk($dari_tanggal, $sampai_tanggal)->result_array();
                ...
                $data['stokMasuk']        = $this->M_barang->report_barang_masuk($dari_tanggal, $sampai_tanggal)->result_array();
                $this->load->view('report/cetakStokMasuk', $data);
            }
        }
    }
